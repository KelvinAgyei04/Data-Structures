...
V=w*(L/2-x)
m='the shear force at x=0 is'
n='the shear force at x=0 is'
print()
print('(a.1)'+ m + str(M)+ 'and',n + str(V))
#Bending moment(M) and shear force(V) at the first end,x=L10
x=L
M=(w*(-6*x**2+6*L*x-L**2))/12
V=w*(L/2-x)
...
b=-L
c=L**2/6
#Using the Almighty formula the formula the two roots are;
...
#Question c
"""
Whn theshear force iszero,x=L/2
"""
x=L/2
print()
...
b=-L
c=(L**2/6)+ (2*m_AM)/w
#Using the almighty formual the two roots are;
discriminant=b**2-4*a*c
root_1f=(-b+ np.sqrt(discriminant))/2*a
root_2f=(-b- np.sqrt(discriminant))/2*a
print()
print('(f)the points along L at which the absolute values of the ending moment array is minimum are {0} and {1}'.format(root_1f,root_2f))
...
When the bending moment is max_M,we get an expresion x**2-Lx+(L**2/6)+(2*max_M)/w=0
"""
